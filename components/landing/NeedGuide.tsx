...
export function NeedGuide({
  ...
  async function onSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      await track("form_start", { type: "guide" });
      await submitLead({
        type: "guide",
        name: data.get("name"),
      ...
      await track("lead_submitted", { type: "guide", rec: encoded.preferredTime });
      setStep("result");
    } catch (err) {
          ...
          <div className="guide-choices">
            {GUIDE_SIZE_OPTIONS.map((option) => (
              <button
                key={option.id}
                type="button"
                aria-pressed={answers.sizeBand === option.id}
          ...
          <div className="guide-choices">
            {question.options.map((option) => (
              <button
                key={option.id}
                type="button"
                aria-pressed={answers[step as "who"] === option.id}
